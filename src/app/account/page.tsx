...
export default function AccountPage() {
  ...
  if (!user) return <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-background">
    ...
    <Button asChild className="rounded-2xl h-12 px-8"><a href="/">Вернуться на главную</a></Button>
  </div>;

  if (profile?.isBanned) return <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-background">
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case "new": return { label: "Принят", color: "bg-blue-500/10 text-blue-500" };
      case "processing": return { label: "В работе", color: "bg-orange-500/10 text-orange-500" };
      case "shipped": return { label: "Отправлен", color: "bg-indigo-500/10 text-indigo-500" };
      case "delivered": return { label: "Получен", color: "bg-emerald-500/10 text-emerald-500" };
              ...
              <CardContent className="pt-0 -mt-16 text-center">
                ...
                <div className="mt-8 pt-8 border-t border-white/10 text-left space-y-4">
                  ...
                  <div className="flex justify-between items-center px-2">
                    ...
                    <span className="text-lg font-black">{profile?.totalOrders || 0}</span>
                  </div>
                </div>
              ...
              <TabsContent value="orders" className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {orders.length === 0 ? (
                  <Card className="rounded-[2.5rem] border-white/20 p-20 text-center bg-background/40 backdrop-blur-xl border-dashed border-2">
                    ...
                    <Button asChild className="rounded-2xl h-12 px-8"><a href="/#catalog">Перейти к покупкам</a></Button>
                  </Card>
                ) : (
                          ...
                          <CardHeader className="flex flex-row items-center justify-between pb-2 bg-muted/20">
                            <div>
                              ...
                              <CardDescription className="mt-1 font-medium">
                                {new Date(order.createdAt?.seconds * 1000).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}
                              </CardDescription>
                            </div>
                              ...
                              <div className="grid md:grid-cols-2 gap-8">
                                ...
                                <div className="flex flex-col justify-between items-end">
                                  <div className="text-right space-y-2">
                                    ...
                                    <p className="text-sm font-bold">{order.deliveryMethod === 'cdek' ? 'СДЭК Доставка' : 'Самовывоз Воронеж'}</p>
                                    <p className="text-xs text-muted-foreground">{order.address || 'Центральный пункт выдачи'}</p>
                                  </div>
                                  
                                  {order.status === 'delivered' && !hasReview(order.id) && (
                                    <Dialog open={isReviewModalOpen && selectedOrderId === order.id} onOpenChange={(open) => {
                                      ...
                                      <DialogContent className="rounded-[2.5rem] bg-background/80 backdrop-blur-2xl">
                                        <DialogHeader>
                                          ...
                                          <p className="text-muted-foreground">Поделитесь впечатлениями о заказе #{order.id.slice(-6)}</p>
                                        </DialogHeader>
